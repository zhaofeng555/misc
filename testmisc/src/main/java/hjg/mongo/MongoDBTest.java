...
//		mongo = new Mongo("192.168.0.220", 27017);
...
//		db.authenticate("test", "test".toCharArray());
	...
	public void testConditionQuery() {
		...
		DBObject oneUser = user.findOne(new BasicDBObject("_id", new ObjectId(
				"50a1ed9965f413fa025166db")));
		print(oneUser);

		// 查询age=24
		List<DBObject> userList1 = user.find(new BasicDBObject("age", 24))
				.toArray();
		print("        find age=24: ");
		printList(userList1);

		// 查询age>=23
		List<DBObject> userList2 = user.find(
				new BasicDBObject("age", new BasicDBObject("$gte", 23)))
				.toArray();
		print("        find age>=23: ");
		printList(userList2);

		// 查询age<=20
		List<DBObject> userList3 = user.find(
				new BasicDBObject("age", new BasicDBObject("$lte", 20)))
				.toArray();
		print("        find age<=20: ");
		printList(userList3);

		// 查询age!=25
		List<DBObject> userList4 = user.find(
		...
		// 查询age in[23,24,27]
		List<DBObject> userList5 = user.find(
				new BasicDBObject("age", new BasicDBObject(QueryOperators.IN,
						new int[] { 23, 24, 27 }))).toArray();
		print("        find agein[23,24,27]: ");
		printList(userList5);

		// 查询age not in[23,24,27]
		List<DBObject> userList6 = user.find(
				new BasicDBObject("age", new BasicDBObject(QueryOperators.NIN,
						new int[] { 23, 24, 27 }))).toArray();
		print("        find age not in[23,24,27]: ");
		printList(userList6);

		// 查询29>age>=20
		List<DBObject> userList7 = user.find(
				new BasicDBObject("age", new BasicDBObject("$gte", 20).append(
						"$lt", 29))).toArray();
		...
		// 查询所有的用户，并按照年龄升序排列
		List<DBObject> userList9 = user.find()
				.sort(new BasicDBObject("age", 1)).toArray();
	...
	public void testUpdateUser() {
		...
		BasicDBObject set1 = new BasicDBObject("$set", new BasicDBObject(
				"name", "jindazhong").append("age", 123));
		user.update(query, // 需要修改的数据条件
				set1,// 需要赋的值
				false,// 数据如果不存在,是否新建
				false);// false只修改第一条,true如果有多条就不修改
	...
	public void testInitTestData() {
		...
		zhangguochen.put("interest", new String[] { "hadoop", "study",
				"mongodb" });
		DBObject zhangguochenAddress = new BasicDBObject();
		zhangguochenAddress.put("address", "henan");
