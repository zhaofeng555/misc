...
public class TestExecutor {
	...
	public static void test1() {
		 ...
		 * 当一个新任务被提交时,若然没有空闲线程,则会暂时存储在一个任务队列中, 待有空闲的线程便处理任务队列中的任务
		 */
		threadPool = Executors.newFixedThreadPool(10);

		/*
		 * 该方法返回一个只有一个线程的线程池,当一个新任务被提交后, 会进入任务队列待有空闲线程时即处理任务队列中的任务
		 */
		threadPool = Executors.newSingleThreadExecutor();

		/*
		 * 该方法返回一个可根据实际任务数量调整线程数量的线程池, 该线程池的线程数量是不固定的,若有空闲线程,则会优先处理任务,
		...
		/*
		 * 提交一个有返回值（计算结果）的任务， 温馨提示，如果任务需要返回计算结果使用service.submit()
		...
		/*
		 * 当线程池使用完毕可以使用shutdown关闭线程池， shutdown温柔的关闭方式，等待任务执行结束完毕后关闭线程池
		 * shutdownNow马上关闭
		 */
		service.shutdown();
